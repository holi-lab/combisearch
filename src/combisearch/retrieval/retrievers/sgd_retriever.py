...
class SGDRetriever(ExampleRetriever):
    """Dense (SBERT + KDTree) retriever for the SGD setting.
    """
    ...
    def data_item_to_embedding(self, data_item, **kwargs) -> NDArray:
        with torch.no_grad():
            embed = self.model.encode(
                self.string_transformation(data_item), convert_to_numpy=True
            ).reshape(1, -1)
        return embed
    ...
    def item_to_best_examples(self, data_item, k=5, decoder: AbstractExampleListDecoder = TopKDecoder()):
        ...
        try:
            return decoder.select_k(
                k=k,
                examples=(
                    (self.label_to_data_item(turn_label), score)
