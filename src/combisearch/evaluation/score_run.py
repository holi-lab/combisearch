...
from combisearch.representation.types import CompletionParser, Turn, MultiWOZDict
from tqdm import tqdm

from combisearch.evaluation.error_analysis import read_running_log_file
from combisearch.evaluation.dst import evaluate, evaluate_on_domain
from combisearch.domain.normalization import AbstractNormalizer
from combisearch.prompting.generator import get_completion_parser
...
def score_running_log(running_log, test_set, turn=-1, verbose=True) -> Dict[str, Any]:
    # turn = -1 means evalute all dialogues
    # turn = 0 means evaluate single-turn dialogues
    # turn = 1 means evalute two-turn dialogues... etc.

    result_dict: Dict[int, List[int]] = defaultdict(list)
    ...
    for data_item, label_item in tqdm(zip(running_log, test_set), disable=not verbose):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prompt-format', type=str, default="nl-prompt-chat",
    ...
    score_running_log(running_log, test_set)
