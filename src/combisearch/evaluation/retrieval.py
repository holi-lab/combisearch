...
def compute_fbeta(gold: List[str], pred: List[str], beta: float = 1) -> float:
    TP, FP, FN = 0, 0, 0
    if len(gold) != 0:
        count = 1
        for g in gold:
...
def compute_sv_sim(gold, pred, onescore=True, beta: float = 1):
    ...
    value_sim = compute_fbeta(gold, pred, beta=beta)

    gold = ['-'.join(g.split('-')[:2]) for g in gold]
    pred = ['-'.join(g.split('-')[:2]) for g in pred]
    slot_sim = compute_fbeta(gold, pred)
...
def evaluate_single_query_ex(turn, retriever, beta: float = 1):
    examples = retriever.item_to_best_examples(turn, k=5)
...
def evaluate_retriever_on_dataset(dataset, retriever, beta: float = 1):
    ...
    return _safe_mean(turn_value_sims), _safe_mean(turn_slot_sims), _safe_mean(all_value_sims), _safe_mean(all_slot_sims)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_fn', type=str, required=True, help="training data file (few-shot or full shot)")
