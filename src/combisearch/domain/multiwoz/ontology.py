...
from combisearch.representation.types import SlotName, SlotValue
...
def insert_space(token, text):
    """Insert spaces around occurrences of `token` in `text`, skipping digit-flanked ones."""
    sidx = 0
    while True:
...
def normalize(text: str) -> str:
    """Lowercase and canonicalize MultiWOZ surface text (whitespace, punctuation, common variants)."""
    text = text.lower()
...
def parse_schema(schema_json: List[Dict[str, Any]]) -> Schema:
    """
    Index `schema.json` contents by slot name. Input format:
...
class Ontology:
    ...
    def get_canonical(self, full_slot_name: SlotName, value: SlotValue) -> Optional[SlotValue]:
        """
        Convert value to its canonical form for full_slot_name (e.g. 'hotel-name'): the form defined in the
        original database for the entity it references. E.g. surface forms 'the acorn guest house',
        'acorn guest house', 'the acorn guesthouse' all de-reference to 'acorn guest house', as defined in
        domain/multiwoz/db/hotel_db.json. value need not be a name; categories and timestamps are also handled
        (e.g. '5:14' -> '05:14'). Returns None if there is no canonical form, which implies value is not in the
        ontology.
        """
    ...
    @staticmethod
    def create_ontology(min_fuzzy_match: int = 90) -> "Ontology":
        known_values: Dict[SlotName, Set[SlotValue]] = {}

        schema = parse_schema(json.loads(read_package_text("domain/multiwoz/db/schema.json")))

        domain_dbs = {
            domain: json.loads(read_package_text(f"domain/multiwoz/db/{domain}_db.json"))
        ...
        for full_slot_name in get_args(SlotName):
            ...
            schema_domain, schema_slot = schema_slot_name.split('-')
        ...
        return Ontology(known_values, schema=schema, min_fuzzy_match=min_fuzzy_match)
